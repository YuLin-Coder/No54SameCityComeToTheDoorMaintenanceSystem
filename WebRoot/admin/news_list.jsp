...
<div class="wangid_conbox">
	...
	<!-- 筛选 --> 
	<form action="newsListSearch.do" method="post" >
	<div class="shuaix">
		<div class="right">
			<input type="text" name="name" placeholder="请输入新闻的名称">
	...
	<table class="layui-table" lay-filter="mylist" lay-size="lg">
		<thead>
			<tr>
				...
				<th lay-data="{field:'yx', align:'center',minWidth:170}">名称</th>
				<th lay-data="{field:'time',align:'center', minWidth:100}">发布时间</th>
		...
		<tbody>
		 ...
		 <tr>
			  ...
			  <c:if test="${sessionScope.auser.utype=='管理员' }">
			 	<a class="layui-btn layui-btn-xs"   href="doUpdateNews.do?id=${u.id }" >修改</a>
				<a class="layui-btn layui-btn-danger layui-btn-xs color2"  onclick="return del(1,1,1)" href="deleteNews.do?id=${u.id }" >删除</a>
			  </c:if>
			  <c:if test="${sessionScope.auser.utype!='管理员' }">
			 	<a class="layui-btn layui-btn-xs"   href="doUpdateNews.do?id=${u.id }" >查看</a>
			  </c:if>
			  </td>
			</tr>
        </c:forEach> 
